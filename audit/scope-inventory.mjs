...
try {
  for (const [name, url] of Object.entries(sources)) {
    ...
    assert.equal(inventory.questions.length,358);
    const retired=new Set(JSON.parse(fs.readFileSync(path.join(root,'audit/retired-questions.json'))).questions.map(q=>q.number));
    assert.deepEqual(inventory.questions.map(q=>q.number).sort((a,b)=>a-b),Array.from({length:419},(_,i)=>i+1).filter(n=>!retired.has(n)));
