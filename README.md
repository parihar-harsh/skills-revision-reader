...
The deployment also includes `roadmap.html`, a focused AI / GenAI Engineer route derived only from the provided course syllabus for freshers who meet its Python, Git and command-line prerequisites but are starting AI from zero. It contains 64 direct YouTube videos: 54 main-course lessons (29 hours 27 minutes) and 10 bonus interview/career lessons, for 35 hours 39 minutes in Full coverage. Resources are mostly Hindi/Hinglish; English fallbacks are labeled and used only where topic-by-topic CampusX searches found no suitable direct lesson. Ten weekly build gates and 44 episode outputs cover every learner-facing week, assignment, capstone and resume-evidence outcome in the syllabus, including the speech-to-text bridge required by the interview-coach capstone. Transformer and attention mathematics, backpropagation, gradient descent, loss functions, foundation-model training, heavy RLHF/DPO, distributed training, CNNs, RNNs and classical deep-learning detours remain excluded as specified by the syllabus.

The route defaults to Full coverage and presents five phases across LLM applications, RAG systems, agentic systems, production readiness and interviews. Fast track contains the main eight-week technical course; Full coverage adds only the syllabus's two bonus interview and job-search weeks. Modules now follow Weeks 1-10 exactly, including observability before deployment and separate interview and career weeks. Every video has a stable lesson number, each week includes a collapsible assignment/evidence panel and ends with a locked build gate, and the next-lesson panel shows the learner's exact route position. Search, completed-item filtering, remaining-time estimates, responsive layouts, dark mode and browser-saved progress make the page usable as a self-contained study and practice sequence rather than only a link collection.
