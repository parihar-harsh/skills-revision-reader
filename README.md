...
`live-coding.html` adds a searchable, role-filtered list of broad practical patterns derived from candidate-reported live coding, machine coding, and timed assessment tasks in frontend, backend, databases, DevOps, and CS fundamentals. Every pattern includes an instant-revision core idea, a focused variable-length list of concepts needed to reconstruct or defend that implementation, and a concise inline code or command reference. Interview-experience and GeeksforGeeks sources remain recorded in `live-coding-data.js`; inferred practice areas, take-home assignments, broad theory dumps, and DSA problems are excluded.
...
Resume-aligned role filters group the same verified questions for SDE I, frontend/React, backend/Node.js, full-stack JavaScript, MERN, AI/GenAI engineering, and DevOps positions. Selecting a role does not duplicate or invent questions.
