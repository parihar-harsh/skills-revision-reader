...
The deployment also includes `roadmap.html`, a focused AI / GenAI Engineer route aligned to the provided course syllabus for freshers who meet its Python, Git and command-line prerequisites but are starting AI from zero. It contains 47 essential videos (27 hours 15 minutes) and 4 classical-ML interview-awareness videos in Hindi or Hinglish, all as direct YouTube links. Nine build checkpoints cover LLM APIs, structured output, production prompting, LangChain, foundational and advanced RAG, evaluation, tool-using agents, LangGraph, multi-agent systems, MCP, FastAPI, Docker deployment, observability, caching, security, an Unsloth fine-tuning run, system design, resume framing, portfolio evidence and job outreach. Transformer mathematics, foundation-model training, research, data science, computer vision and advanced MLOps are deliberately excluded.

The route defaults to Full coverage and presents five phases across LLM applications, RAG systems, agentic systems, production readiness and interviews. Every video has a stable lesson number, each module ends with a locked checkpoint, and the next-lesson panel shows the learner's exact route position. Search, Fast track/Full coverage filters, completed-item filtering, remaining-time estimates, responsive layouts, dark mode and browser-saved progress make the page usable as a self-contained study and practice sequence rather than only a link collection.
