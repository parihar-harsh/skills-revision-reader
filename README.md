...
`live-coding.html` adds a searchable, role-filtered list of broad practical patterns derived from candidate-reported live coding, machine coding, and timed assessment tasks in frontend, backend, databases, DevOps, and CS fundamentals. Every pattern includes an instant-revision core idea, four must-prepare concepts, and a concise inline implementation or command reference, while its interview-experience and GeeksforGeeks sources remain recorded in `live-coding-data.js`; inferred practice areas, take-home assignments, and DSA problems are excluded.
